...
try:
    ...
    _, input_extension = os.path.splitext(input_basename)
    ...
    output_root, _ = os.path.splitext(output_basename)

    with open(Path(input_basename).name, 'rb') as file:
    ...
    with open(Path(output_basename).name, 'wb+') as f:
        while data:
            f.write(data)
            data = sock.recv(output_size if output_size <= STREAM_RATE else STREAM_RATE)
